...
def parse_args():
    usage = '\n1. You can compute character similarity by:\n' \
            'python get_similar_char_edit.py 午 牛 年 千\n' \
            '\n' \
            '2. You can use ted in computing character similarity by:\n' \
            'python get_similar_char_edit.py 午 牛 年 千 -t\n' \
            '\n'
...
if __name__ == '__main__':
    ...
    print(c.shape_similarity("合", "洽"))
    print(c.shape_similarity("倚", "椅"))
    print(c.shape_similarity("任", "仟"))
    word_sets = set()
    import pickle
    ...
    with open("./tmp" + str(val) + ".txt", "w", encoding="utf=8") as f:
        for i, c1 in enumerate(char_li):
            for c2 in char_li[i:]:
                if c1 != c2 and c.shape_similarity(c1, c2) > val:
                    f.write(c1 + "\t" + c2 + ":" + str(c.shape_similarity(c1, c2)) + "\n")
                    print(c1 + "\t" + c2 + ":" + str(c.shape_similarity(c1, c2)) + "\n")
                    # print(f'For character pair ({c1}, {c2}):')
                    # print(f'    v-sim = {c.shape_similarity(c1, c2)}')
                    # print(f'    p-sim = {c.pronunciation_similarity(c1, c2)}\n')

    with open("./data/word_set.txt", "w", encoding="utf=8") as fw:
        for s in word_sets:
            fw.write(s + "\n")
