...
with open("/data_local/plm_models/chinese-macbert-base/vocab.txt", "r", encoding="utf-8") as f:
...
class Trainer:
    ...
    def train(self, train, gradient_accumulation_steps=1):
        ...
        for batch in train:
            ...
            ori_c_loss = 0.3 * outputs[1] + 0.7 * outputs[0]
    ...
    def test(self, test):
        ...
        for batch in test:
            ...
            c_loss = 0.3 * outputs[1] + 0.7 * outputs[0]
        ...
        for batch in test:
            ...
            for i in range(num):
                ...
                for j in range(len(tokens) + 1):
                    if out[i][j + 1] != input_ids[i][j + 1] and out[i][j + 1] != 100:
                        val = out[i][j + 1].item()
                        # print(val)
                        if j < len(tokens):
...
if __name__ == "__main__":
    ...
    #
    bert_path = "/data_local/plm_models/chinese-macbert-base/"
