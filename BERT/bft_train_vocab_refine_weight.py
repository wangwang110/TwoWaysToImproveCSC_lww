...
import operator
from model import BertFineKeep, construct, BertDataset, BFTLogitGen, readAllConfusionSet
import os
import random
import pickle
...
class Trainer:
    ...
    def trg2vec(self, src, trg, max_seq_length):
        ...
        for a, b in zip(tokens_a, tokens_b):
            if a == b:
                input_ids.append(self.cls_w2id["KEEP"])
...
if __name__ == "__main__":
    ...
    model = BertFineKeep(bert, tokenizer, device, device_ids).to(device)
