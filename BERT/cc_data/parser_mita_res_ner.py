    ...
    for line2 in f2.readlines():
        ...
        for item in list(trg_pre.strip()):
            ...
            if i == num:
                break
            i += 1
        all_model_res_ner[key] = trg_text

# 最终写入文件
path_out = "/data_local/TwoWaysToImproveCSC/BERT/data_analysis/" + task + "_ner_4.txt"
with open(path_out, "w", encoding="utf-8") as fw3:
    for key in all_mita_res:
        ...
        if key in all_model_res and key in all_model_res_ner:
            model_text = all_model_res[key]
            model_text_ner = all_model_res_ner[key]
            ...
            #     fw3.write("=================\n\n")
                ...
                fw3.write("model_ner:\n")
                for j, x in enumerate(zip(src_text, model_text_ner)):
                    s, t = x
                    if s != t:
                        fw3.write(str(j + 1) + "," + s + "," + t + "\n")
