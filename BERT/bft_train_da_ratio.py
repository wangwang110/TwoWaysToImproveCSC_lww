...
class Trainer:
    ...
    def replace_token_ratio(self, lines):
        """
        # 随机选取id，查看id对应的词是否在候选集中
        # 如果在90%替换为候选词，10%随机选取token（来自哪里呢？统计字典）
        # 直到1/4的字符被替换
            ...
            for i in index_li:
                if tokens[i] in self.confusion_set:
                    ...
                    if np.random.rand(1) > 0.9:  # 这个比例是否要控制
                        ...
                        token_conf_set = self.confusion_set[tokens[i]]
                        if tokens[i] in self.confusion_s and tokens[i] in self.confusion_p:
                            if np.random.rand(1) > 0.8:
                                token_conf_set = self.confusion_s[tokens[i]]
                            else:
                                token_conf_set = self.confusion_p[tokens[i]]
                        idx = np.random.randint(0, len(token_conf_set))
                        tokens[i] = list(token_conf_set)[idx]
                    if count == up_num:
                        break
    ...
    def train(self, train):
        ...
        for batch in train:
            ...
            if "pretrain" in args.task_name:
                generate_srcs = self.replace_token_ratio(batch['output'])
