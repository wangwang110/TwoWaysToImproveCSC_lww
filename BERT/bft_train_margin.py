...
class Trainer:
    def __init__(self, bert, optimizer, tokenizer, device):
        ...
        self.criterion_c = CombineLoss(p=2)
        # nn.MultiMarginLoss
        # 和所有类别比较
        # 仅仅和原始类别比较
        # MarginLoss()
    ...
    def train(self, train):
        ...
        for batch in train:
            ...
            c_loss = self.criterion_c(out.reshape(batch_size * seq_len, -1), input_ids.reshape(batch_size * seq_len),
                                      output_ids.reshape(batch_size * seq_len))
