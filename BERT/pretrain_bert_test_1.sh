...
train_data=./data/wiki_00_base_0.train
save_path=$base/wiki_00_base_zuowen_vocab/
mkdir $save_path
CUDA_VISIBLE_DEVICES="1" python bft_pretrain_mlm.py  --task_name=test --gpu_num=1 --load_model=False --do_train=True --train_data=$train_data --bert_path=$bert_path --do_valid=True --valid_data=$valid_data --epoch=10 --batch_size=20 --learning_rate=2e-5 --do_save=True --save_dir=$save_path --seed=100 > $save_path/bft_train.log 2>&1 &


#train_data=./data/wiki_00_base_sep_1.train
#save_path=$base/wiki_00_base_sep_1/
#mkdir $save_path
...
#
#
#train_data=./data/wiki_00_base_sep_2.train
#save_path=$base/wiki_00_base_sep_2/
#mkdir $save_path
