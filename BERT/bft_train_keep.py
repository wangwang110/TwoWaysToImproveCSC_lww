...
class Trainer():
    ...
    def attack(self, test, ratio=0.15, batch_size=20):
        ...
        ad_gen = []
        for batch in test:
            gen, count, mod_count = BFTLogitGen(batch['output'], batch['output'], self.model, tokenizer, device,
                                                self.confusion_set, ratio)
            ad_gen += gen
    ...
    def ad_train(self, train, ratio):
        ...
        for batch in train:
            if (count % 3 != 0):
                ...
                ad_sample_gen, err_count, mod_count = BFTLogitGen(batch['output'], batch['output'], self.model,
                                                                  self.tokenizer, self.device, self.confusion_set,
                                                                  ratio)
